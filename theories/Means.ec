...
theory Mean.
  ...
  proof strict.
    ...
    fun; seq 1 : (v = x) (mu_x d v) pr 1%r 0%r ((glob A)=(glob A){m})=> //.
      ...
      call (_: (glob A) = (glob A){m} /\ x = v ==> 
             ev v (glob A) res) => //. simplify pr. 
      bypr => &m' eqGlob.
      equiv_deno (_: ={glob A, x} ==> ={res, glob A}) => //; by fun true. 
    conseq* (_: _ ==> false) => //.
  qed.
  ...
  proof.
    ...
    rewrite /cpOrs or_exists;split.
      ...
      split => //. 
      by rewrite img_def;exists (fst (res{m1}));progress => //;smt.
    intros [x ].     
    by rewrite img_def => /= [ [v [<- /= Hm] [H1 <- ]]].
  save.
  ...
  proof.
    ...
    elimT set_ind (Finite.toFSet (create (support d))).
