(* -------------------------------------------------------------------- *)
require import AllCore List FSet Distr DProd DList StdOrder StdBigop.
(*---*) import Bigreal Bigreal.BRM MUnit.

op [opaque] dfset (d : 'a distr) (n : int): 'a fset distr =
  dcond (dmap (dlist d n) oflist) (fun fs => card fs = n).

lemma dfset_def (d : 'a distr) n: dfset d n = 
  dcond (dmap (dlist d n) oflist) (fun fs => card fs = n) by rewrite/dfset.

(* FIXME: move *)
lemma dscale_of_ll ['a] (d: 'a distr) :
  is_lossless d => 
  dscale d = d.
proof.
rewrite /dscale /is_lossless => LL; rewrite LL; apply dscalar1.
qed.

lemma dscale_dunit ['a] (x: 'a) :
  dscale (dunit x) = dunit x.
apply dscale_of_ll; apply dunit_ll.
qed.

lemma drestrict_dunit ['a] (x: 'a) (p: 'a -> bool) :
  drestrict (dunit x) p =
    if p x then dunit x else dnull.
admitted.

search dnull.

lemma dscale_dnull ['a] :
  dscale dnull<:'a> = dnull.
proof. by rewrite /dscale weight_dnull invr0; apply dscalar0r. qed.

(* FIXME: move *)
lemma dcond_dunit ['a] (x: 'a) (p: 'a -> bool) :
  dcond (dunit x) p = 
    if p x then dunit x else dnull.
proof.
  rewrite /dcond drestrict_dunit;
  case (p x) => P; 1: apply dscale_dunit; 1: apply dscale_dnull.
qed.

lemma dfset0 (d : 'a distr) n: n <= 0 => dfset d n = dunit (fset0).
proof. 
move => ge0_n; rewrite dfset_def /fset0 dlist0; 1:apply ge0_n; 1:rewrite dmap_dunit dcond_dunit. cbv. 
search dcond.
qed.

print FSet.

lemma dfset1 (d : 'a distr) : dfset d 1 = dmap d fset1.
proof.
by rewrite dfset_def /fset1 dlist1 dmap_comp //.
qed.

(*
lemma dfsetS (d : 'a distr) n:
  0 <= n =>
  dfset d (n + 1)
...
op djoin_fset ['a] (ds: 'a Distr.distr fset) : 'a fset Distr.distr =
  


lemma dfset_djoin (d : 'a distr) n: 0 <= n => dlist d n = djoin (nseq n d).
proof.
