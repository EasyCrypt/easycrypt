...
(* ==========================================================================
   ...
   finite-bound requirement in the predicate [dominated M d1 d2], which
   asserts [M >= 0] and [mu1 d1 x <= M * mu1 d2 x] pointwise.  Under
   [dominated M d1 d2] every lemma below goes through; without it the
   ...
   Contents:
     ...
     Section 3      — Structural lemmas: dmap, dlet, dprod, dlist,
                      ...
                      dfst, dsnd, dopt, dlet' (bilinear), dfold.
     [RDivFun]      — abstract theory: [dominated_dfun], [rdiv_inf_dfun]
                      (clone with [theory FT <- YourFinType]).
     [Distinguisher] — abstract theory: [Sample] game, [adv_rdiv_inf],
                      fused variants (dmap, dlet, dexcepted, dcond).
     [DistinguisherList] — abstract theory: [adv_rdiv_inf_dlist].

   Companion files:
     [SmoothRDiv.ec]  — smooth max-divergence (Rényi + sdist hop).
     [RDivOracle.ec]  — oracle-access bound via PROM ([rdiv_inf_oracleRO]).
...
proof.
move => dom.
apply rdiv_inf_le_ub; first exact (rdiv_inf_ge0 M _ _ dom).
move => b; rewrite !dmap1E.
exact (rdiv_inf_pr M _ _ _ dom).
qed.
...
move => dom.
apply rdiv_inf_le_ub; first exact (rdiv_inf_ge0 M _ _ dom).
by move => y; exact (dlet_pointwise _ _ _ _ y (rdiv_inf_dominated M _ _ dom)).
qed.
...
- by apply mulr_ge0; [exact (rdiv_inf_ge0 Ml _ _ doml) |
                       exact (rdiv_inf_ge0 Mr _ _ domr)].
move => p.
exact (dprod_pointwise _ _ _ _ _ _ p
         (rdiv_inf_dominated Ml _ _ doml) (rdiv_inf_dominated Mr _ _ domr)).
qed.
...
proof.
move => ge0_n dom.
have ge0_rdiv : 0%r <= rdiv_inf d1 d2 by exact (rdiv_inf_ge0 M _ _ dom).
elim: n ge0_n => [|n ge0_n IHn].
- rewrite !dlist0 // RField.expr0 rdiv_inf_dd //.
  by rewrite dunit_ll.
rewrite !dlistS // RField.exprS //.
have dom_dlist := dominated_dlist M _ _ n ge0_n dom.
apply (ler_trans (rdiv_inf (d1 `*` dlist d1 n) (d2 `*` dlist d2 n))).
- by apply (rdiv_inf_dmap (M * RField.exp M n));
     exact (dominated_dprod M (RField.exp M n) _ _ _ _ dom dom_dlist).
apply (ler_trans (rdiv_inf d1 d2 * rdiv_inf (dlist d1 n) (dlist d2 n))).
- exact (rdiv_inf_dprod M (RField.exp M n) _ _ _ _ dom dom_dlist).
...
lemma dominated_djoinmap ['a 'b] (M : real) (F1 F2 : 'a -> 'b distr) (xs : 'a list) :
...
- by rewrite /djoinmap /= RField.expr0; exact dominated_refl.
have dom_head := dom_cons x _; first by rewrite mem_head.
have dom_tail : dominated (RField.exp M (size xs)) (djoinmap F1 xs) (djoinmap F2 xs).
- by apply IHxs => y y_in; apply dom_cons; rewrite in_cons y_in.
rewrite /djoinmap /= !djoin_cons addzC RField.exprS ?size_ge0 //.
apply dominated_dmap.
exact (dominated_dprod M (RField.exp M (size xs)) _ _ _ _ dom_head dom_tail).
qed.

lemma rdiv_inf_djoinmap ['a 'b] (M : real) (F1 F2 : 'a -> 'b distr) (xs : 'a list) :
  (forall x, x \in xs => dominated M (F1 x) (F2 x)) =>
...
have dom_head := dom_cons x _; first by rewrite mem_head.
have dom_tail_all : forall y, y \in xs => dominated M (F1 y) (F2 y).
- by move => y y_in; apply dom_cons; rewrite in_cons y_in.
have dom_tail : dominated (RField.exp M (size xs)) (djoinmap F1 xs) (djoinmap F2 xs)
  by apply dominated_djoinmap.
have ge0_head : 0%r <= rdiv_inf (F1 x) (F2 x) by exact (rdiv_inf_ge0 M _ _ dom_head).
have ge0_tail_rdiv : 0%r <= rdiv_inf (djoinmap F1 xs) (djoinmap F2 xs)
  by exact (rdiv_inf_ge0 (RField.exp M (size xs)) _ _ dom_tail).
rewrite /djoinmap /= !djoin_cons BRM.big_cons /predT /=.
apply (ler_trans
  (rdiv_inf (F1 x `*` djoinmap F1 xs) (F2 x `*` djoinmap F2 xs))).
- by apply (rdiv_inf_dmap (M * RField.exp M (size xs)));
     exact (dominated_dprod M (RField.exp M (size xs)) _ _ _ _ dom_head dom_tail).
apply (ler_trans
  (rdiv_inf (F1 x) (F2 x) * rdiv_inf (djoinmap F1 xs) (djoinmap F2 xs))).
- exact (rdiv_inf_dprod M (RField.exp M (size xs)) _ _ _ _ dom_head dom_tail).
...
proof.
elim: ds1 ds2 => [|d1 ds1 IH] [|d2 ds2] /=.
- rewrite /dominated RField.expr0 djoin_nil; smt(ge0_mu1).
- smt(size_ge0).
- smt(size_ge0).
move => eq_sz dom_pt.
have eq_sz' : size ds1 = size ds2 by smt().
have dom_head : dominated M d1 d2.
- have h := dom_pt 0 _; first smt(size_ge0).
  by move: h => /=.
have dom_tail : dominated (RField.exp M (size ds1)) (djoin ds1) (djoin ds2).
- by apply IH => // i i_bound; have := dom_pt (i + 1) _; smt().
rewrite !djoin_cons addzC RField.exprS ?size_ge0 //.
apply dominated_dmap.
exact (dominated_dprod M (RField.exp M (size ds1)) _ _ _ _ dom_head dom_tail).
qed.
...
proof.
elim: ds1 ds2 => [|d1 ds1 IH] [|d2 ds2] /=.
- rewrite djoin_nil range_geq // BRM.big_nil.
  rewrite rdiv_inf_dd //; smt(dunit_ll).
- smt(size_ge0).
- smt(size_ge0).
move => eq_sz dom_pt.
have eq_sz' : size ds1 = size ds2 by smt().
...
- have h := dom_pt 0 _; first smt(size_ge0).
  by move: h => /=.
have dom_tail_all : forall i, 0 <= i < size ds1 =>
    dominated M (nth witness ds1 i) (nth witness ds2 i).
- move => i i_bound; have := dom_pt (i + 1) _; first smt().
  smt().
have dom_tail : dominated (RField.exp M (size ds1)) (djoin ds1) (djoin ds2)
  by apply (dominated_djoin M) => //.
have ge0_head : 0%r <= rdiv_inf d1 d2 by exact (rdiv_inf_ge0 M _ _ dom_head).
rewrite !djoin_cons range_ltn; first smt(size_ge0).
rewrite BRM.big_cons /predT /=.
apply (ler_trans (rdiv_inf (d1 `*` djoin ds1) (d2 `*` djoin ds2))).
- by apply (rdiv_inf_dmap (M * RField.exp M (size ds1)));
     exact (dominated_dprod M (RField.exp M (size ds1)) _ _ _ _ dom_head dom_tail).
apply (ler_trans (rdiv_inf d1 d2 * rdiv_inf (djoin ds1) (djoin ds2))).
- exact (rdiv_inf_dprod M (RField.exp M (size ds1)) _ _ _ _ dom_head dom_tail).
apply ler_wpmul2l => //.
apply (ler_trans
  (BRM.big predT (fun i => rdiv_inf (nth witness ds1 i) (nth witness ds2 i))
                  (range 0 (size ds1)))).
- by apply IH.
rewrite (addzC 1 (size ds1)) range_addr BRM.big_map /(\o) /=.
apply lerr_eq; apply BRM.eq_big_seq => i hi /=.
by congr; smt(nth_behead behead_cons mem_range).
qed.
...
case => [|y]; rewrite !dopt1E /=.
- by rewrite eq_w; smt(mu_bounded).
apply (ler_trans (rdiv_inf d1 d2 * mu1 d2 y)); first exact (rdiv_inf_upper_bound M _ _ y dom).
apply ler_wpmul2r; smt(ge0_mu1).
qed.
...
lemma dominated_dlet' ['a 'b] (Md Mf : real) (d1 d2 : 'a distr) (F1 F2 : 'a -> 'b distr) :
  dominated Md d1 d2 =>
  (forall x, dominated Mf (F1 x) (F2 x)) =>
  dominated (Md * Mf) (dlet d1 F1) (dlet d2 F2).
proof.
case => [ge0_Md dom_d] dom_F.
have [ge0_Mf _] := dom_F witness.
split; first by apply mulr_ge0.
move => y; rewrite !dlet1E -sumZ.
apply ler_sum => [x /= | |].
- have h1 := dom_d x.
  have [_ h2] := dom_F x; have h2' := h2 y.
...
(* Dominance of [dfold] under step-wise uniform domination.  The per-step
   bound [Ms i] uniformly dominates the i-th step's kernel across all
   accumulator values.  Conclusion: the product of step-wise bounds. *)
...
apply (dominated_dlet' (BRM.big predT Ms (range 0 n)) (Ms n) _ _ _ _ IH).
by move => y; split => //; exact (step_n y).
qed.
...
(* -- dfun ----------------------------------------------------------------
   ...
   automatically.  Your [FinType] clone must use [<=] for type/op
   substitutions — see [RDivOracle.ec] header for details. *)
...
abstract theory RDivFun.
  ...
  lemma dominated_dfun ['u] (M : real) (F1 F2 : FT.t -> 'u distr) :
  ...
  rewrite !dfun_dmap.
  apply dominated_dmap.
  apply (dominated_djoinmap M) => x _; exact (dom_pt x).
  qed.
  ...
  move => dom_pt.
  rewrite !dfun_dmap.
  apply (ler_trans (rdiv_inf (djoinmap F1 FT.enum) (djoinmap F2 FT.enum))).
  - apply (rdiv_inf_dmap (RField.exp M (size FT.enum))).
    apply (dominated_djoinmap M) => x _; exact (dom_pt x).
  apply (rdiv_inf_djoinmap M) => x _; exact (dom_pt x).
  qed.
...
- by apply (adv_rdiv_inf A _ _ M); apply (dominated_dmap M).
apply ler_wpmul2r; first by rewrite Pr [mu_ge0].
exact (rdiv_inf_dmap M _ _ G dom).
qed.
...
- by apply (adv_rdiv_inf A _ _ M); apply (dominated_dlet M).
apply ler_wpmul2r; first by rewrite Pr [mu_ge0].
exact (rdiv_inf_dlet M _ _ G dom).
qed.
...
- by apply (adv_rdiv_inf A _ _ (1%r / (weight d - mu d Q)));
     apply dominated_dexcepted.
apply ler_wpmul2r; first by rewrite Pr [mu_ge0].
...
- by apply (adv_rdiv_inf A _ _ (1%r / mu d Q)); apply dominated_dcond.
apply ler_wpmul2r; first by rewrite Pr [mu_ge0].
...
apply (ler_trans
   ...
   * Pr[DL.Sample(A).main(dlist d2 n) @ &m : P res])).
- by apply (DL.adv_rdiv_inf A _ _ (RField.exp M n));
     apply (dominated_dlist M).
apply ler_wpmul2r; first by rewrite Pr [mu_ge0].
exact (rdiv_inf_dlist M _ _ _ ge0_n dom).
qed.
