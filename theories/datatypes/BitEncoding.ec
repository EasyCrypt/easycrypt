...
lemma int2bs_strikeE (l n : int) :
  let k = index false (int2bs (l+1) n) in

     0 <= l
  => 0 <= n < 2^l
  => int2bs (l+1) n = nseq k true ++ false :: drop (k+1) (int2bs (l+1) n).
proof.
...
- exists (n %/ (2^k)); rewrite {1}[n](@divz_eq _ (2^k)); congr.
  ...
  - have h := bs2int_le2Xs (int2bs k n).
    rewrite bs2int_ge0 /= (@ltr_le_trans _ _ _ h).
  ...
  have := before_index witness false (int2bs (l+1) n) i rgi.
  by rewrite nth_mkseq 1:/# /= => /negbFE ->.
have oddq: !odd q.
- rewrite oddPn (_ : q %% 2 = (n %/ 2^k) %% 2).
  - rewrite nE divzMDl 1:expf_eq0 // pdiv_small //=.
    by smt(expr_gt0).
  have ->: (n %/ 2^k %% 2 = 0) = !(nth witness (int2bs (l+1) n) k).
...
rewrite drop_oversize ?size_int2bs 1:/# /=; congr.
