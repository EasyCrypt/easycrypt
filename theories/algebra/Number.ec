...
theory Axioms.
  axiom ler_norm_add (x y : t): `|x + y| <= `|x| + `|y|.
  axiom addr_gt0     (x y : t): zeror < x => zeror < y => zeror < (x + y).
  axiom norm_eq0     (x   : t): `|x| = zeror => x = zeror.
  axiom ger_leVge    (x y : t): zeror <= x => zeror <= y => (x <= y) \/ (y <= x).
  axiom normrM       (x y : t): `|x * y| = `|x| * `|y|.
  axiom ler_def      (x y : t): x <= y <=> `|y - x| = y - x.
  axiom ltr_def      (x y : t): x < y <=> (y <> x) /\ x <= y.
  axiom real_axiom   (x   : t): zeror <= x \/ x <= zeror.
  axiom minrE        (x y : t): minr x y = if x <= y then x else y.
  axiom maxrE        (x y : t): maxr x y = if y <= x then x else y.
end Axioms.
...
lemma normr0: `|zeror| = zeror.
proof. by apply/ger0_norm/lerr. qed.

lemma normr1: `|oner| = oner.
proof. by apply/ger0_norm/ler01. qed.

lemma normr_nat n : 0 <= n => `|ofint n| = ofint n.
...
lemma normr_unit : forall x, unit x => unit `|x|.
proof.
...
lemma normrV : forall x, `|invr x| = invr `|x|.
proof.
move=>x.
case: (unit x) => ux.
...
(* -------------------------------------------------------------------- *)
lemma mono_inj f : mono2 f (<=) (<=) => injective f.
proof. by move=> mf x y; rewrite eqr_le !mf -eqr_le. qed.

lemma nmono_inj f : mono2 f (fun y x => x <= y) (<=) => injective f.
proof. by move=> mf x y; rewrite eqr_le !mf -eqr_le. qed.

lemma lerW_mono f : mono2 f (<=) (<=) => mono2 f (<) (<).
proof.
...
lemma ler_pimull x y : zeror <= y => x <= oner => x * y <= y.
proof. by move=> hy hx; rewrite -{2}(mul1r y) ler_wpmul2r. qed.

lemma ler_nimull x y : y <= zeror => x <= oner => y <= x * y.
proof. by move=> hy hx; rewrite -{1}(mul1r y) ler_wnmul2r. qed.

lemma ler_pimulr x y : zeror <= y => x <= oner => y * x <= y.
proof. by move=> hy hx; rewrite -{2}(mulr1 y) ler_wpmul2l. qed.

lemma ler_nimulr x y : y <= zeror => x <= oner => y <= y * x.
...
lemma exprn_ege1 n x : 0 <= n => oner <= x => oner <= exp x n.
proof.
move=> nge0 xge1; elim: n nge0 => [|n nge0 ih]; 1: by rewrite expr0.
by rewrite exprS // mulr_ege1.
qed.

lemma exprn_egt1 n x : 0 <= n => oner < x => (oner < exp x n) <=> (n <> 0).
proof.
...
lemma ler_eexpr x n : 0 < n => oner <= x => x <= exp x n.
proof.
...
lemma ltr_eexpr x n : 0 <= n => oner < x => (x < exp x n <=> 1 < n).
proof.
...
lemma ler_wiexpn2l x : zeror <= x <= oner =>
...
lemma ieexprn_weq1 x n : 0 <= n => zeror <= x =>
...
lemma ieexprIn x : zeror < x => x <> oner =>
  forall m n, 0 <= m => 0 <= n => exp x m = exp x n => m = n.
proof.
(* FIXME: wlog *)
