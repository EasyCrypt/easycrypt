(* ==================================================================== *)
require import AllCore List Ring Int RingStruct SubRing Bigalg StdOrder.
require (*--*) Bigop.
(*---*) import IntOrder.
...
abstract theory ComRingModule.
  ...
  clone import ZModule as Mod with
    type t <= t.
  ...
  theory CRM.
    ...
    lemma scale0r a :
      RL.zeror ** a = Mod.zeror.
    proof. by apply/(addrI (RL.zeror ** a)); rewrite -scaleDl !addr0. qed.
  
    lemma scaler0 (x : scalar) :
      x ** Mod.zeror = Mod.zeror.
    proof. by apply/(addrI (x ** Mod.zeror)); rewrite -scaleDr !addr0. qed.
    ...
    clone import BigZModule as BigMod with
      theory ZM <- Mod.
    ...
    proof.
      elim: ss => [|s ss IHss]; [by rewrite BAdd.big_nil BigMod.big_nil scale0r|].
      by rewrite BAdd.big_cons BigMod.big_cons; case: (P s) => // _; rewrite scaleDl IHss.
    qed.
  
    lemma scalel_sumr ['a] (P : 'a -> bool) (F : 'a -> Mod.t) s (vs : 'a list) :
      s ** BigMod.big P F vs = BigMod.big P (fun v => s ** F v) vs.
    proof.
      elim: vs => [|v vs IHvs]; [by rewrite !BigMod.big_nil scaler0|].
      by rewrite !BigMod.big_cons; case: (P v) => // _; rewrite scaleDr IHvs.
    qed.
    ...
    proof. by rewrite /= BigMod.big_cons /predT /idfun. qed.
  
    lemma lin_cat ss1 ss2 vs1 vs2 :
      size ss1 = size vs1 =>
      lin (ss1 ++ ss2) (vs1 ++ vs2) = lin ss1 vs1 + lin ss2 vs2.
    proof. by move => eq_size; rewrite zip_cat // map_cat BigMod.big_cat. qed.
  
    lemma lin0 n vs :
      lin (nseq n RL.zeror) vs = Mod.zeror.
    proof.
    ...
    proof.
      rewrite scalel_sumr zip_mapl !BigMod.big_mapT.
      apply/BigMod.eq_big_seq => -[s' v] /mem_zip [mem_s' mem_v].
    ...
    proof.
      ...
      exists
        (map
          (fun v => BigRL.BAdd.big (fun (p : scalar * t) => p.`2 = v) fst (zip ss vs))
          (undup vs))
        (undup vs).
      rewrite size_map /= undup_uniq /=; split.
      + by apply/allP => x; rewrite mem_undup => mem_x; move/allP: all_p => ->.
      rewrite zip_mapl_ss !BigMod.big_mapT.
      rewrite BigMod.big_mapT BigMod.big_pairr unzip2_zip in eq_v.
      + by rewrite eq_size.
      rewrite -eq_v => {v eq_v}; apply/BigMod.eq_big_seq => v.
    ...
    proof.
      ...
      move: eq_size eq_ => <- ->>; exists (BigRL.BAdd.big predT idfun ss).
      elim: ss => [|s ss IHss].
      + by rewrite lin_nil1 BigRL.BAdd.big_nil scale0r.
      rewrite /= addrC nseqS ?size_ge0 // lin_cons BigRL.BAdd.big_cons.
    ...
    lemma gen_opp p v :
      gen p v =>
      gen p (-v).
    proof. by move => /(gen_scale _ (-RL.oner) _); rewrite scaleN. qed.
  
    lemma gen_p p v :
      p v =>
      gen p v.
    proof. by move => pv; exists [RL.oner] [v]; rewrite lin_cons lin_nil1 scale1r addr0. qed.
    ...
    lemma gen_t_pred0 :
      gen_t pred0 <=> (forall v , v = Mod.zeror).
    ...
    lemma free0 p :
      ...
      ! p Mod.zeror.
    proof.
      move => /(_ [RL.oner] [Mod.zeror]); rewrite lin_cons lin_nil1 /=.
    ...
    lemma free_pred1 v :
      free (pred1 v) <=> (forall s , s ** v = Mod.zeror => s = RL.zeror).
    proof.
    ...
    lemma free_gen p v :
      free p => p v => gen p v.
    proof.
      by move => free_p p_v; exists [RL.oner] [v]; rewrite /= p_v /= scale1r big_seq1 /idfun.
    qed.
    ...
    lemma free_split p1 p2 :
      free p1 <=>
      ( (forall v , gen (predI p1 p2) v => gen (predI p1 (predC p2)) v => v = Mod.zeror) /\
        free (predI p1 p2) /\
    ...
    lemma free_predU1 p v :
      ...
      ( (p v \/ (forall s , gen p (s ** v) => s = RL.zeror)) /\
        free p).
    proof.
    ...
    lemma gen_t_vf_oflist_nil :
      gen_t (vf_oflist []) <=> (forall v , v = Mod.zeror).
    ...
    lemma free_vf_oflist_cons v vs :
      free (vf_oflist (v :: vs)) <=>
      ( (v \in vs \/ (forall s , gen (vf_oflist vs) (s ** v) => s = RL.zeror)) /\
        free (vf_oflist vs)).
    proof.
