...
abstract theory PolyFF.
  ...
  proof.
    ...
    by rewrite /monic lc0 eq_sym F.oner_neq0.
  qed.
  ...
  lemma eqI_size_enum_iudeg n :
    0 < n =>
    Counting_Argument.I n FinType.card = size (enum_iudeg (n + 1)).
  proof.
    rewrite -(subrK n 1); pose m:= n - 1; move: m => {n} n /ltzS.
    elim/sintind: n => n le0n /= IHn.
    move: (Counting_Argument.eqI (n + 1) _ _ card_gt1); [by apply/ltzS|].
    ...
    have ->: sI = sE; [rewrite /sI /sE|move/addIr]; move=> {sI sE}.
...
abstract theory FFIrrPolyExt.
  ...
  proof.
    ...
    + apply/mapP; exists (modp (repr x) p).
      ...
      rewrite PolyComRing.subr_eq -ulc_divp_eq // -FF.F.unitfE.
  ...
  lemma cardP :
    FFQ.FinType.card = FF.FinType.card ^ (deg p - 1).
  proof.
    rewrite /FFQ.FinType.card size_map size_enum_ledeg //.
