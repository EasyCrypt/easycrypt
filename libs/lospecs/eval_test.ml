...
type op = {
  ...
  args  : (int * [`U | `S]) list;
  out   : [`U | `S];
  def   : Ast.adef; (* feed this into eval_adef *)
  reff  : int list -> int;
}
...
let test (op : op) =
  let vs =
    let bw_of_arg ((sz, s) : int * [`U | `S]) =
      match s with `U -> urange sz | `S -> srange sz 
    in List.map bw_of_arg op.args
  in
  ...
  let func = (Evaluator.eval_adef op.def) in (* change this *)

  let conv_args = (fun vs -> List.combine op.args vs |> 
    List.map (fun ((sz, s), v) -> 
    (Evaluator.from_int v sz ~signed:(match s with | `U -> false | `S -> true)))) in

  let test (vs : int list) =
    let out = conv_args vs |> func in
    let out =
      match op.out with
      | `S -> Evaluator.sbw_to_sbint out |> Z.to_int
      | `U -> fst out |> Z.to_int in
...
(* -------------------------------------------------------------------- *)
let test_uextend (ast: (Ast.symbol * Ast.adef) list) =
  let op (isize : int) (osize : int) : op =
    { name = (Format.sprintf "uextend<%d,%d>" isize osize)
    ; args = [(isize, `U)]
    ; out  = `U
    ; def  = List.find (fun x -> (compare (fst x) "UEXT") == 0) ast |> snd
...
(* -------------------------------------------------------------------- *)
let test_sextend (ast: (Ast.symbol * Ast.adef) list) =
  let op (isize : int) (osize : int) : op =
    { name = (Format.sprintf "sextend<%d,%d>" isize osize)
    ; args = [(isize, `S)]
    ; out  = `S
    ; def  = List.find (fun x -> (compare (fst x) "SEXT") == 0) ast |> snd
    ; reff = (fun vs -> as_seq1 vs)
    }
...
(* -------------------------------------------------------------------- *)
let test_shift8 (ast: (Ast.symbol * Ast.adef) list) ~(side : [`L | `R]) ~(sign : [`U | `S]) =
  ...
  let op (size : int) : op =
    ...
    ; args = [(size, sign); (4, `U)]
    ; out  = sign
    ; def  = List.find (fun x -> (compare (fst x) 
        (match side, asign with | (`L, _) -> "SLL8" | (`R, `A) -> "SRA8" | (`R, `L) -> "SRL8")
    ) == 0) ast |> snd 
...
let test_shift14 (ast: (Ast.symbol * Ast.adef) list) ~(side : [`L | `R]) ~(sign : [`U | `S]) =
  let str_side = match side with `L -> "left" | `R -> "right" in
  let str_sign = match sign with `U -> "u" | `S -> "s" in
  ...
  let op (size : int) : op =
    ...
    ; out  = sign
    ; def  = List.find (fun x -> (compare (fst x) 
        (match side, asign with | (`L, _) -> "SLL14" | (`R, `A) -> "SRA14" | (`R, `L) -> "SRL14")
    ) == 0) ast |> snd 
    ; reff = (fun vs -> let x, y = as_seq2 vs in sim x y)
    }
...
(* -------------------------------------------------------------------- *)
let main () =
  let prog = File.with_file_in "/home/gustavo/Thesis/bdep/easycrypt/libs/lospecs/eval_test.spec" (Io.parse "/home/gustavo/Thesis/bdep/easycrypt/libs/lospecs/eval_test.spec") in
  ...
  List.iter (fun f -> f ()) (List.map (fun x -> (fun () -> (x ast))) tests)
(*
(* ==================================================================== *)
