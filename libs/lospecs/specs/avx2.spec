...
VPMADDWD_256(w1@256, w2@256) -> @256 =
  map<32, 8>(
    fun w1@32 w2@32 .
      add<32>(
        smul<16>(w1[@16|0], w2[@16|0]),
        smul<16>(w1[@16|1], w2[@16|1])
      ),
    w1,
    w2
  )
...
VPSLLV_8u32(w1@256, counts@256) -> @256 =
  map<32, 8>(
    fun w1@32 count@32 .
    ugt<32>(0x0020@32, count) ? sll<32>(w1, count[@8|0]) : 0,
    w1,
    counts
  )
...
VPSLL_16u16(w@256, count@8) -> @256 =
  map<16, 16>(sll<16>(., count), w)
...
VPEXTRACTI32_8(w@256, i@8) -> @32 = 
  w[@32|i[@3|0]]

VPEXTRACTI32_4(w@128, i@8) -> @32 = 
  w[@32|i[@2|0]]

VPEXTRACTI32_2(w@64, i@8) -> @32 = 
  w[@32|i[0]]
