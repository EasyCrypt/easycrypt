...
module FromSpec () : S = struct
  (* ------------------------------------------------------------------ *)
  let specs =
    let specs = match Sys.getenv_opt "EC_AVX2_SPEC_FILE_PATH" with
    | Some s -> s
    | None -> Format.eprintf "Path to avx2 spec file not set, please set env var EC_AVX2_SPEC_FILE_PATH with the correct path to the file@.";
      exit 1
    in
