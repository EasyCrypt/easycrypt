...
/* -------------------------------------------------------------------- */
value value_of_w128(__m128i x) {
  CAMLparam0();
  CAMLlocal1(out);

  out = caml_alloc_tuple(2);
  Store_field(out, 0, caml_copy_int64(_mm_extract_epi64(x, 0)));
  Store_field(out, 1, caml_copy_int64(_mm_extract_epi64(x, 1)));

  CAMLreturn(out);
}

/* -------------------------------------------------------------------- */
__m128i w128_of_value(value x) {
  CAMLparam1(x);

  __m128i out = _mm_set_epi64x(
    Int64_val(Field(x, 1)),
    Int64_val(Field(x, 0))
  );

  CAMLreturnT(__m128i, out);
}

/* -------------------------------------------------------------------- */
__m256i _mm256_inserti128_si256_dyn(__m256i a, __m128i b, const int imm8) {
  switch (imm8 & 0x01) {
    case 0:
...
/* -------------------------------------------------------------------- */
__m128i _mm256_extracti128_si256_dyn(__m256i a, const int imm8) {
  switch (imm8 & 0x01) {
    case 0:
...
/* -------------------------------------------------------------------- */
__m256i _mm256_blend_epi16_dyn(__m256i a, __m256i b, const int imm8) {
#define CASE(I) case I: return _mm256_blend_epi16(a, b, I)
...
/* -------------------------------------------------------------------- */
__m256i _mm256_blend_epi32_dyn(__m256i a, __m256i b, const int imm8) {
#define CASE(I) case I: return _mm256_blend_epi32(a, b, I)
...
/* -------------------------------------------------------------------- */
__m256i _mm256_permute4x64_epi64_dyn(__m256i a, const int imm8) {
#define CASE(I) case I: return _mm256_permute4x64_epi64(a, I)
...
/* -------------------------------------------------------------------- */
__m256i _mm256_permute2x128_si256_dyn(__m256i a, __m256i b, const int imm8) {
#define CASE(I) case I: return _mm256_permute2x128_si256(a, b, I)
...
/* -------------------------------------------------------------------- */
__m256i _mm256_moveldup_ps_dyn(__m256i a) {
   return (__m256i)_mm256_moveldup_ps((__m256)a);
}
...
BIND2(mm256_permute4x64_epi64_dyn, M256i, M256i, Long);
BIND3(mm256_permute2x128_si256_dyn, M256i, M256i, M256i, Long);
...
BIND2(mm256_srli_epi16, M256i, M256i, Long);

BIND1(mm256_movemask_epi8, Int32, M256i);
BIND1(mm256_moveldup_ps_dyn, M256i, M256i); 

BIND3(mm256_blend_epi16_dyn, M256i, M256i, M256i, Long);
BIND3(mm256_blend_epi32_dyn, M256i, M256i, M256i, Long);


BIND3(mm256_inserti128_si256_dyn, M256i, M256i, M128i, Long);
BIND2(mm256_extracti128_si256_dyn, M128i, M256i, Long);
}
