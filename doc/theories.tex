...
\section{The \EasyCrypt Expression Language\label{sec:types}}
...
For example, Listing~\ref{lst:tuples} declares two abstract types, \rawec{pkey}
and \rawec{skey}, and then defines a type \rawec{keypairs} as the type of pairs
of elements of \rawec{pkey} and \rawec{skey}. Intepreting types as sets of
values, the \rawec{*} on types morally corresponds to the cartesian product.
...
\begin{easycrypt}[label={lst:tuples}]{An example of tuple type.}
type pkey.
type skey.
