...
Following the set-based analogy and standard functional programming notation, we
denote with \ec{f : 'a -> 'b} the fact that \ec{f} is a function from
type \ec{'a} to type \ec{'b}. (Note the similarity with
$f \in \alpha \rightarrow \beta$.) Given function \ec{f} typed as above, and an
\ec{x : 'a}, we denote with \ec{f x} the value (of type \ec{'b}) resulting from
applying \ec{f} to \ec{x}.

Functions expecting several arguments take them separated by spaces.
...
integer arguments and return an integer.

Were \ec{log} given the type \ec{log: (int * int) -> int}, the expression
above would be written \ec{log (2,n)}. \ec{('a * 'b)} is the type of pairs
of elements of \ec{'a} and \ec{'b}, and the type \ec{(int * int) -> int} should
therefore be read as the type of functions that take, as their
\emph{only argument}, a pair of integers, and return an integer.
