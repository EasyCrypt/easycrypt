...
The \rawec{unit} type contains a unique element \rawec{tt}.
...
expressions. The former evaluates to $0$ (the division is integer division, and
the cast is applied to the result) whereas the latter evaluates to $0.5$ (the
division is division in the real numbers). Eventually, we will introduce
notations for scoping that should allow less cluttered real expressions.}
