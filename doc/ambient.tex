...
\futurebox{For now you can only introduce something that appears in your goal but in a near future you will ba able to
...
%Tactics working with the locals
...
%clear
\addLogic{clear}{\tacarg{h}{ident}?}{It removes the judgement \vararg{h} from the context
\begin{displaymath}
...
%smt
\addAuto{smt}{\tacarg{db}{lident} \tacarg{max}{int}? \tacarg{time}{int}?  \tacarg{names}{string*}}{
This tactic send your goal to provers.\\
You can give as first argument nolocals and your local context($\Gamma$) won't be sent to smt. The second
argument permits to precise some information, the number of provers run at the same time.\\
\futurebox{The argument sgould evolve in the future, you will be able to specify which lemma you want to send
to provers. The number of simultaneous prover you want to run at the same time should not appears in scripts
and the limitation time should be just an indicator on the diffuclty of the lemma to proof.}\\
...
%elimT
\addAuto{elimT}{\tacarg{h}{ident} \tacarg{x}{value}}{Apply an induction principle \vararg{h} on object \vararg{x}}

%subst
\addAuto{subst}{\tacarg{x}{ident}?}{Find a value for var \vararg{x} in the hypothesis, subsitute it everywhere and remove it.
If \vararg{x} is not specify it does it for all variable.}
...
{idtac}
{\tacarg{x}{string}}
{This tactic does nothing except printing the string put as argument if there is one.}

%print
\addOther
{print}
{\cstarg{type} | \cstarg{op} | \cstarg{theory} | \cstarg{pred} | \cstarg{axiom} ~ \tacarg{x}{ident}}
{This tactic takes a type of object and such an object ant print it.}
