...
(* Why obligation from file "easycryptaec023.why", line 423, characters 0-2538: *)
(*Why goal*) Lemma todo_in_coq : 
  (forall (U_146:Z),
   (forall (U'_145:Z),
    (forall (z_144:Z),
     (forall (pg_143:Z),
...
Proof.
 intros.
 assert (W:= minus_mult_mod_0 _ _ _ H H0 H1 H2 H3 H4 H5). 
 split; simpl_mod.
 match goal with |- math_mod ?x _ = _ => 
  replace x with 
    ( ((inv_int_mod_q ((U'_145 - U_146) * pg_143)) *
       ((U'_145 - U_146) * pg_143)) * Rc_142)%Z;[ | ring ]
   end.
 rewrite  mult_mod_idemp_l, inv_correct, Zmult_1_l;
   auto using mod_small with zarith.
 match goal with |- math_mod ?x _ = _ =>
  replace x with
   ( (U_146 * z_144 + aux_141) + 
    (inv_int_mod_q ((U'_145 - U_146) * pg_143) * ((U'_145 - U_146) * pg_143)) *
     (Rc_142 - U_146 * z_144 - aux_141)); [ | ring ] end.
  rewrite plus_mod_idemp_r, mult_mod_idemp_l, inv_correct,
     <- plus_mod_idemp_r;auto.
  match goal with |- math_mod ?x _ = _ => ring_simplify x end.
  apply mod_small;omega.
Save.
