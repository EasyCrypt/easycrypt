    ...
    (0 <= Rap_164 ->
     (Rap_164 <= (q - 1) ->
      (1 <= pg_163 ->
       (pg_163 <= (q - 1) ->
        (math_mod
         ((math_mod (pg_163 * Rap_164) q) * (inv_int_mod_q pg_163)) q) =
        Rap_164)))))).
Admitted.


(* Why obligation from file "easycrypt50aa28.why", line 493, characters 0-354: *)
Ltac push_mod t :=
 match t with
    ...
    (0 <= Rap_168 ->
     (Rap_168 <= (q - 1) ->
      (1 <= pg_167 ->
       (pg_167 <= (q - 1) ->
        (math_mod
         (pg_167 * (math_mod (Rap_168 * (inv_int_mod_q pg_167)) q)) q) =
        Rap_168)))))).
Proof.
 intros;simpl_mod.
 intros;simpl_mod.
 replace (pg_167 * (Rap_168 * inv_int_mod_q pg_167))
 with ((inv_int_mod_q pg_167 * pg_167) * Rap_168);[ | ring].
