...
(*Why logic*) Definition Hash_function : Z -> group -> group -> group -> Z.
Admitted.
...
(*Why axiom*) Lemma H_bound :
  (forall (hk_218:Z),
   (forall (g1_217:group),
    (forall (g2_216:group),
     (forall (g3_215:group), 0 <=
      (Hash_function hk_218 g1_217 g2_216 g3_215) /\
      (Hash_function hk_218 g1_217 g2_216 g3_215) <= (q - 1))))).
Admitted.
...
(*Why axiom*) Lemma length_pos :
  (forall (l_220:(list Z)), 0 <= (length_list l_220)).
Admitted.

(* Why obligation from file "easycryptb434e6.why", line 351, characters 0-402: *)
(*Why goal*) Lemma todo_in_coq6 : 
  (forall (Ly2_222:(list Z)),
   (Rle
    (Rmult
     (real_of_bool (le_int_bool (length_list Ly2_222) qD)) (Rdiv
                                                            (IZR
                                                             (length_list
                                                              Ly2_222)) (
                                                            IZR (q - 1 + 1))))
...
Proof.
 intros.
 assert (0 <= IZR qD)%R.
  rewrite <- real_of_int_zero;apply IZR_le;apply qD_pos.
 assert (0 < IZR q)%R.
  rewrite <- real_of_int_zero;apply IZR_lt;apply q_pos.
 case_eq (le_int_bool (length_list Ly2_222) qD);intros.
 replace (q - 1 + 1) with q;[ | ring].
 rewrite real_of_bool_true;[rewrite Rmult_1_l | trivial].
 rewrite le_int_bool_axiom in H1.
 apply Rmult_le_compat;auto using IZR_le with real.
 rewrite <- real_of_int_zero;apply IZR_le;apply length_pos.
 rewrite real_of_bool_false;trivial.
 rewrite Rmult_0_l;apply Rmult_le_pos;auto with real.
Save.
