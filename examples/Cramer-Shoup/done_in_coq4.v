...
(* Why obligation from file "easycryptd3922d.why", line 559, characters 0-3505: *)
(*Why goal*) Lemma todo_in_coq4 : 
  (forall (x_210:Z),
   (forall (x1_209:Z),
    (forall (x2_208:Z),
     (forall (y_207:Z),
               ...
               (forall (u'_197:Z),
                (forall (r_196:Z),
                 (forall (a_195:group),
                  (forall (ap_194:group),
                   (forall (d_193:group),
                    (forall (Ly2_192:(list Z)),
                     (x2_208 =
                      (math_mod
                       ((Rd_203 - U_202 * (x_210 + V_200 * y_207)) *
                       (inv_int_mod_q ((U'_201 - U_202) * pg_204)) - V_200 *
                       y2_205) q) ->
                      (x1_209 = (math_mod (x_210 - pg_204 * x2_208) q) ->
                       (y1_206 = (math_mod (y_207 - pg_204 * y2_205) q) ->
                        (0 <= y2_205 ->
                         (y2_205 <= (q - 1) ->
                          (1 <= pg_204 ->
                           (pg_204 <= (q - 1) ->
                            (~ap_194 = (group_pow a_195 pg_204) ->
                             (~vd_199 = V_200 ->
                              (0 <= vd_199 ->
                               (vd_199 <= (q - 1) ->
                                (0 <= V_200 ->
                                 (V_200 <= (q - 1) ->
                                  (u_198 = (dl_g a_195) ->
                                   (u'_197 =
                                    (math_mod
                                     ((dl_g ap_194) * (inv_int_mod_q pg_204)) q) ->
                                    (d_193 =
                                     (group_mul
                                      (group_pow
                                       a_195 (x1_209 + vd_199 * y1_206)) (
                                      group_pow
                                      ap_194 (x2_208 + vd_199 * y2_205))) ->
                                     (r_196 = (dl_g d_193) ->
                                      (in_list
                                       y2_205 (Cons
                                               (math_mod
                                                (((r_196 - u_198 *
                                                 ...
                                                 (inv_int_mod_q
                                                  ((U'_201 - U_202) * pg_204))) *
                                                (inv_int_mod_q
                                                 (vd_199 - V_200))) q) Ly2_192)) =
                                      true)))))))))))))))))))))))))))))))))))).
Proof.
 intros.
 rewrite in_list_Cons, bool_or_def, poly_eq_eq;left.
 assert (Hpg_mod:math_mod pg_204 q = pg_204) by (rewrite mod_small;omega).
 assert (Hpg0: 0 <> math_mod pg_204 q) by omega.
 assert (r_196 = math_mod (u_198 * (x1_209 + vd_199 * y1_206) + (u'_197 * pg_204) * (x2_208 + vd_199 * y2_205)) q).
  rewrite H15, H14, <- pow_dl_g;apply f_equal.
  rewrite <- (dl_g_pow a_195), <- (dl_g_pow ap_194).
  repeat rewrite group_pow_mult; rewrite group_pow_mult_distr.
  rewrite H12, H13.
  rewrite group_pow_mod;symmetry;rewrite group_pow_mod;apply f_equal.
  transitivity 
   (math_mod
    (dl_g a_195 * (x1_209 + vd_199 * y1_206) +
    dl_g ap_194 * math_mod (inv_int_mod_q pg_204 * pg_204) q *
    (x2_208 + vd_199 * y2_205)) q).
  simpl_mod;symmetry;simpl_mod.
  apply f_equal2;[ring | trivial].
  rewrite inv_correct, Zmult_1_r;trivial.
 rewrite mult_mod_idemp_l, minus_mod.
 match goal with |- _ = math_mod ( math_mod (?x - ?y) _ * _) _ => 
  replace x with (math_mod (x2_208 + vd_199 * y2_205) q);
  [ replace y with (math_mod (x2_208 + V_200 * y2_205) q) | ]
 end.
 symmetry;simpl_mod.
 match goal with |- ?x = _ =>
  replace x with (math_mod (y2_205 * math_mod (inv_int_mod_q (vd_199 - V_200) * (vd_199 - V_200)) q) q)end.
 rewrite inv_correct, Zmult_1_r, mod_small;auto with zarith.
 apply not_eq_sym;apply minus_mod_diff;auto.
 simpl_mod;apply f_equal2;[ ring| trivial].
 rewrite H;simpl_mod;apply f_equal2;[ring | trivial].
 symmetry;rewrite H16, H0, H1;simpl_mod. simpl_mod.
 match goal with |- math_mod ?x _ = _ =>
  replace x with ((x2_208 + vd_199 * y2_205) * 
     (inv_int_mod_q ((u'_197 - u_198) * pg_204) * ((u'_197 - u_198) * pg_204)));[ | ring] end.
 rewrite mult_mod_idemp_r, inv_correct, Zmult_1_r;trivial.
 assert (W1 : 0 <= u_198 <= q - 1) by (rewrite H12;apply dl_g_bound).
 assert (W2 : 0 <= u'_197 <= q - 1).
  rewrite H13.
  assert (W:= math_mod_bound (dl_g ap_194 * inv_int_mod_q pg_204) q).
  assert (W' := abs_int_pos q).
  auto with zarith.
 destruct W1;destruct W2; apply not_eq_sym;apply minus_mult_mod_0;auto.
 intros Heq;apply H6.
 rewrite <- (dl_g_pow a_195), <- (dl_g_pow ap_194).
 rewrite (group_pow_mod g (dl_g ap_194)).
 rewrite group_pow_mult, (group_pow_mod g (dl_g a_195 * pg_204));apply f_equal.
 transitivity 
    (math_mod ((dl_g ap_194)*math_mod (inv_int_mod_q pg_204 * pg_204) q) q).
 rewrite inv_correct, Zmult_1_r;auto.
 simpl_mod;rewrite Zmult_assoc, mult_mod_idemp_l, <- H13, <- H12, Heq;trivial.
Save.
