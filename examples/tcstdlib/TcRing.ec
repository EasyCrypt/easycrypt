...
(* -------------------------------------------------------------------- *)
require import Core Int TcMonoid.

(* -------------------------------------------------------------------- *)
type class group <: monoid = {
  op [ - ] : group -> group

  axiom addNr: left_inverse idm [-] (+)<:group>
}.

section.
declare type g <: group.
...
section.
declare type r <: comring.

instance monoid with r
  op idm = oner<:r>
  op (+) = ( * )<:r>.
realize addmA by exact: mulrA.
realize addmC by exact: mulrC.
realize add0m by exact: mul1r.
...
type class idomain <: comring = {
  axiom mulf_eq0:
    forall (x y : idomain), x * y = zeror <=> x = zeror \/ y = zeror
}.
...
(* -------------------------------------------------------------------- *)
(*
(* TODO: Disjointness of type class operator names? *)
type class ffield <: group = {
  op onef  : ffield
  op ( * ) : ffield -> ffield -> ffield
  op invf  : ffield -> ffield

  axiom onef_neq0 : onef <> zeror
  axiom mulfA     : associative ( * )
  axiom mulfC     : commutative ( * )
  axiom mul1f     : left_id onef ( * )
  axiom mulfDl    : left_distributive ( * ) (+)<:ffield>
  axiom mulVf     : left_inverse_in (predC (pred1 zeror)) onef invf ( * )
  axiom unitP     : forall (x y : ffield), y * x = onef => x <> zeror
  axiom unitout   : invr zeror = zeror
}.
*)
...
(* -------------------------------------------------------------------- *)
(* TODO: Instantiation of type classes with inheritance is broken *)
(* TODO: Instantiation of type class operators with literals is broken *)
op zeroz = 0.
op addz (x y : int) = x + y.
op negz (x : int) = -x.
...
instance group with int
  ...
  op [-] = negz.
realize addNr.
(* TODO: Note that the zero remains undefined *)
rewrite /left_inverse /negz /idm.
(* by exact: addNz. *) admit.
