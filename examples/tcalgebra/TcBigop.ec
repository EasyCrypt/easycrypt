...
(* -------------------------------------------------------------------- *)
lemma big_distrl ['a] (op_ : t -> t -> t) (P : 'a -> bool) F s u:
     left_zero idm op_
  => left_distributive op_ (+)
  => op_ (big P F s) u = big P (fun a => op_ (F a) u) s.
proof.
  move=> mulm1 mulmDl; pose G := fun x => op_ x u.
  move: (big_comp G P) => @/G /= -> //.
    by rewrite mulm1. by move=> t1 t2; rewrite mulmDl.
qed.

lemma big_distrr ['a] (op_ : t -> t -> t) (P : 'a -> bool) F s u:
      right_zero idm op_
   => right_distributive op_ (+)
   => op_ u (big P F s) = big P (fun a => op_ u (F a)) s.
proof.
  move=> mul1m mulmDr; pose G := fun x => op_ u x.
  move: (big_comp G P) => @/G /= -> //.
...
(* -------------------------------------------------------------------- *)
lemma big_andbC (P Q : 'a -> bool) (F : 'a -> t) s:
...
(* -------------------------------------------------------------------- *)
lemma bigU ['a] (P Q : 'a -> bool) (F : 'a -> t) s : (forall x, !(P x /\ Q x)) =>
...
(* -------------------------------------------------------------------- *)
lemma big_pair_pswap ['a 'b] (p : 'a * 'b -> bool) (f : 'a * 'b -> t) s :
    big<:'a * 'b> p f s
...
(* -------------------------------------------------------------------- *)
lemma big1_eq (P : 'a -> bool) s: big P (fun (x : 'a) => idm) s = idm.
proof.
...
(* -------------------------------------------------------------------- *)
lemma big_flatten (P : 'a -> bool) (F : 'a -> t) rr :
...
(* -------------------------------------------------------------------- *)
lemma big_pair ['a 'b] (F : 'a * 'b -> t) (s : ('a * 'b) list) : uniq s =>
  big predT F s =
    big predT (fun a =>
...
(* -------------------------------------------------------------------- *)
lemma big_nseq_cond (P : 'a -> bool) (F : 'a -> t) n x :
...
(* -------------------------------------------------------------------- *)
lemma big_undup ['a] (P : 'a -> bool) (F : 'a -> t) s :
...
(* -------------------------------------------------------------------- *)
lemma exchange_big (P1 : 'a -> bool) (P2 : 'b -> bool) (F : 'a -> 'b -> t) s1 s2:
...
(* -------------------------------------------------------------------- *)
lemma big_allpairs (f : 'a -> 'b -> 'c) (F : 'c -> t) s u:
    big predT F (allpairs<:'a, 'b, 'c> f s u)
  = big predT (fun x => big predT (fun y => F (f x y)) u) s.
proof.
elim: s u => [|x s ih] u //=.
...
(* -------------------------------------------------------------------- *)
lemma big_int_recr_cond (n m : int) P (F : int -> t): m <= n =>
  bigi P F m (n+1) =
...
(* ==================================================================== *)
(* Display wrappers: [bigA] for additive contexts, [bigM] for
   multiplicative ones. Both unfold to [big] so all the lemmas above
   apply transparently. The flavor tag on the carrier ([addmonoid]
   vs [mulmonoid]) drives which wrapper the printer folds back to.     *)
(* ==================================================================== *)
