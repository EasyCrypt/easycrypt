...
op inp  : (secret_key, eph_key) -> message.
op out_noclash : (secret_key, eph_key) -> message.
...
op gen_session_string_sid : 
  (session_id, (part,   secret_key) map, (message * part, eph_key) map) -> session_string.
...
axiom gen_session_string_upd_skey : forall (sid : session_id, skey : (part,   secret_key) map, seed :(message * part, eph_key) map, p : part, sk : secret_key, str : session_string),
! in_dom(p, skey) && 
gen_session_string_sid(sid,skey,seed) = str =>
gen_session_string_sid(sid,upd(skey,p,sk),seed) = str.

axiom gen_session_string_upd_seed : forall (sid : session_id, skey : (part,   secret_key) map, seed :(message * part, eph_key) map, p : message * part, ek : eph_key, str : session_string),
! in_dom(p, seed) && 
gen_session_string_sid(sid,skey,seed) = str =>
...
type session_descr = (part * message * bool * bool).

op mk_session_descr (x : part, y : message, b1 : bool, b2: bool) = 
(x,y,b1,b2). 
op session_part (x : session_descr) = let a,b,c,d = x in a.
op session_msg (x : session_descr) = let a,b,c,d = x in b.
op session_eph_flag (x : session_descr) = let a,b,c,d = x in c.
op session_key_reveal_flag (x : session_descr) = let a,b,c,d = x in d.

op same_session_string_abs : 
(session_id , session_id, (part, secret_key) map, (message * part, eph_key) map)  -> bool.
...
forall (sid1 : session_id, 
        sid2 : session_id, 
        skey : (part, secret_key) map, 
        seed : (message * part, eph_key) map),
gen_session_string_sid(sid1,skey,seed)=gen_session_string_sid(sid2,skey,seed) =>
same_session_string_abs(sid1,sid2,skey,seed).

op eqS_abs : 
  (session_string,  session_id, (part, secret_key) map, (message* part, eph_key) map) -> bool.
...
op findelse_sid_abs : ((session_id, session_key) map,  session_id,(part, secret_key) map, (message * part, eph_key) map) -> session_id option.
...
op findelse_g_abs : ((session_id, session_key) map ,  session_string, (part, secret_key) map, (message * part, eph_key) map) -> session_id option.
...
op findelse_h_abs : ((session_string, session_key) map ,  session_id,(part, secret_key) map, (message * part, eph_key) map) -> session_string option.
...
forall (m :  (session_string, session_key) map,
        ...
        e : eph_key),
 !in_dom((X,p),seed)  =>
 findelse_h_abs(m,sid,skey,seed) = findelse_h_abs(m,sid,skey,upd(seed, (X, p), e)).
...
(in_dom((A,X), complete_session) && 
 let B', Y',flag1,flag2 = complete_session[(A,X)] in
 B = B' && Y = Y').
...
pred fresh(tested_sessions : (session_id, session_key) map,
           ...
           complete_session : complete_session_with_status,
           incomplete_session : incomplete_session_with_status) =
forall (sid :session_id), in_dom(sid,tested_sessions) => fresh_sid(sid,corrupt,complete_session,incomplete_session).
