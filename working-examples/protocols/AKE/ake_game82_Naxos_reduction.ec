...
equiv Reduction : AKE82Naxos.Main ~ AKE82NaxosCDH.Main :
       true ==> 
         ={seedB,incomplete_sessions,complete_sessions,
             pkA,pkB,skA,Ych,alpha,strch} 
          && Ych{1}=U{2}  
          && pkB{1}=V{2} 
          && ((let A', B', AY, BX, XY = strch{1} in
               (A' = pkB{1} && B' = pkA{1} && gdh(A',Ych{1},AY) && 
                gdh(B',alpha{1},BX) && gdh(alpha{1},Ych{1},XY))) => 
               gdh(V{2},U{2},res{2})).
inline.
  auto (={seedB,incomplete_sessions,complete_sessions,pkA,pkB,skA,Ych} &&  
      Ych{1}=U{2}  && pkB{1}=V{2}).
derandomize.
*(rnd >>).
trivial.
...
claim Naxos_CDH_2 :
AKE82Naxos.Main[wingame82_naxos(pkA,pkB,alpha, Ych, strch)] <= AKE82NaxosCDH.Main[gdh(V,U,res)]
using Reduction.

(*AKE82Naxos.Main[wingame82_naxos(pkB,pkA,alpha,Ych,strch)] <= AKE82NaxosCDH.Main[gdh(V,U,res)]*)
