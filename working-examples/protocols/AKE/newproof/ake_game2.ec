...
KeyReveal = { 
 ...
 (* var sidA, sidB : session_id;  YL 13/09/2012*)
 var x : eph_key;
 (A,B,X,Y) = s;
 x = seed[(A,X)];
 ...
 if (in_dom((A,X), complete_sessions) && 
  ...
  { (* (A,_,X,_) is completed*)
   ...
   (*sidA = mk_sid(A, B', X, Y'); YL 13/09/2012*)
   if ( B = B' && Y = Y') 
   {(* B = B' /\ Y = Y'*)
    ...
    {(*B,_,Y,_ not complete*)
     ...
     (*sidB = mk_sid(B, A', Y, X');YL 13/09/2012*)
  ...
  {(* (A,_,X,_) is not completed or tested or matching tested*)
   ssskey = dummy_session_key;
  } 
 return ssskey;
} and
