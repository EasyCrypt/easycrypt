...
Theorem WP_parameter_two_equal_elements : forall (a:Z), forall (n:Z),
  ...
  (i <  a)%Z) -> let result := (get a1 i) in (((0%Z <= result)%Z /\
  (result <  n)%Z) -> ((~ ((get deja_vu result) = true)) ->
  (((0%Z <= result)%Z /\ (result <  n)%Z) -> forall (deja_vu1:(map Z bool)),
  (deja_vu1 = (set deja_vu result true)) -> ((v1 = (-1%Z)%Z) -> forall (v:Z),
  ((0%Z <= v)%Z /\ (v <  n)%Z) -> ~ (appear_twice a2 v (i + 1%Z)%Z)))))))))).
