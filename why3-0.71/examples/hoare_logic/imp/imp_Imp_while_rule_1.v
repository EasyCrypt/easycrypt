...
Axiom while_induction : forall (e:expr) (inv:fmla) (i:stmt),
  (valid_triple (Fand (Fterm e) inv) i inv) -> forall (s1:(map ident Z))
  (s2:(map ident Z)) (loo:stmt) (skip:stmt), (skip = Sskip) ->
  ((loo = (Swhile e i)) -> ((many_steps s1 loo s2 skip) -> ((eval_fmla s1
  inv) -> (eval_fmla s2 inv)))).
...
intros e inv i.
intros Hinv_preserved s Hinv_init s' Hreduction.
generalize (while_induction _ _ _ Hinv_preserved).
intros Hind.

intros s1 s2 l s Hs Hl; subst.

inversion Hreduction; subst; clear Hreduction.
inversion H; subst; clear H.
(* case cond true *)
simpl.
elim (many_steps_seq _ _ _ _ H0).
intros s3 (H1,H2).
