...
let t_hoareF_conseq pre post tc =
  ...
  let (post, epost, d) = post.hsi_inv in
  ...
  in
  let cond2e = EcProofTyping.merge2_poe_list f_imp (fepost,fd) (epost,d) in
...
let t_hoareS_conseq pre post tc =
  ...
  let concl3 =
    f_hoareS (snd hs.hs_m) pre hs.hs_s {hsi_m=m;hsi_inv=(post, epost,d)}
  in
...
let t_conseq pre post tc =
  ...
  | FhoareF hf, Inv_ss pre, Inv_ss post ->
    let (_,epost,d) = (hf_po hf).hsi_inv in
    let post = ss_inv_rebind post (hf_po hf).hsi_m in
    let post = {hsi_m=(hf_po hf).hsi_m;hsi_inv=(post.inv,epost,d)} in
    t_hoareF_conseq pre post tc
  | FhoareS hs, Inv_ss pre, Inv_ss post ->
    let (_,epost,d) = (hs_po hs).hsi_inv in
    let post = ss_inv_rebind post (hs_po hs).hsi_m in
    let post = {hsi_m=(hs_po hs).hsi_m;hsi_inv=(post.inv,epost,d)} in
...
let t_hoareF_notmod post tc =
  ...
  let econd1 = EcProofTyping.merge2_poe_list f_imp (fepost,fd) (epost,d) in
...
let gen_conseq_nm_ss tnm tc pre post =
    FApi.t_internal ~info:"generic-conseq-nm" (fun g ->
      let gs =
        (tnm post @+
          [ t_id;
...
let t_hoareS_conseq_nm   = gen_conseq_nm_ss t_hoareS_notmod   t_hoareS_conseq
...
let t_hoareF_conseq_bdhoare tc =
  ...
  if not (is_empty_poe (hf_po hf).hsi_inv) then
    tc_error !!tc "exception are not supported";
  let post = lower_f (hf_po hf) in
...
let t_hoareF_conseq_equiv f2 p q p2 q2 tc =
  ...
  if not (is_empty_poe (hf_po hf1).hsi_inv) then
    tc_error !!tc "exception are not supported";
  ...
  let (post,_,_) = (hf_po hf1).hsi_inv in
...
let rec t_hi_conseq notmod f1 f2 f3 tc =
  ...
  | FhoareF _,
    ...
    if not (is_empty_poe (hf_po hf2).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let post = lower_f (hf_po hf2) in
    FApi.t_seqsub
  ...
  | FbdHoareS hs0,
    ...
    if not (is_empty_poe (hs_po hs2).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let posta1 = lower_f (hs_po hs2) in
  ...
  | FbdHoareF hs0,
    ...
    if not (is_empty_poe (hf_po hs2).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let posta1 = lower_f (hf_po hs2) in
    let posta  = map_ss_inv2 f_and post posta1 in
  ...
  | FequivS _,
    ...
    if not (is_empty_poe (hs_po hs2).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let post2 = lower_f (hs_po hs2) in
    if not (is_empty_poe (hs_po hs3).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let post3 = lower_f (hs_po hs3) in
  ...
  | FequivS es, Some _, Some _, None ->
    ...
    let post = {hsi_m=m;hsi_inv=empty_poe f_true} in
  ...
  | FequivS es, Some _, None, Some _ ->
    let m = EcIdent.create "&hr" in
    let f2 =
      f_hoareS (snd es.es_ml) {m;inv=f_true} es.es_sl {hsi_m=m;hsi_inv=empty_poe f_true}
    in
  ...
  | FequivF _,
   ...
   if not (is_empty_poe (hf_po hs2).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let post2 = lower_f (hf_po hs2) in
    if not (is_empty_poe (hf_po hs3).hsi_inv) then
      tc_error !!tc "exception are not supported";
    let post3 = lower_f (hf_po hs3) in
  ...
  let process_cut1 ((pre, post, poe), bd) =
     let penv, qenv, env_e, gpre, gpost, ty, fmake =
      ...
      | FhoareS hs ->
        ...
        let fmake pre post c_or_bd =
          ...
          | Some (PCI_bd (cmp, bd)) ->
            if not (is_empty_poe post.hsi_inv) then
              tc_error !!tc "exception are not supported";
            let post = lower_f post in
    ...
    let (pre, post, bd) = match gpre, gpost with
      | Inv_ss gpre, Inv_hs gpost ->
        let bd = bd |> omap (process_info !!tc penv gpre.m) in
        let (gp,gpoe,gd) = gpost.hsi_inv in
        let post = post |> odfl gp in
        let (poe,d) = poe |> odfl (gpoe, gd) in
        (Inv_ss {inv=pre;m=gpre.m}, Inv_hs {hsi_inv=(post,poe,d);hsi_m=gpost.hsi_m}, bd)
    ...
    let penv, qenv, env_e, gpre, gpost, ty, fmake =
      ...
      | FhoareF hf ->
        ...
        | Some f1 ->
          match (snd f1).f_node with
          | FequivF ef when side = `Left ->
            ef.ef_fr, {m; inv=f_true}, {hsi_m=m; hsi_inv=empty_poe f_true}
...
let process_conseq notmod
    ((info1, info2, info3) : conseq_ppterm option tuple3) tc =
...
let t_conseqauto ?(delta = true) ?tsolve tc =
  ...
  let t_hoareF_notmod f = t_hoareF_notmod (lift_f f) in
  let t_hoareS_notmod f = t_hoareS_notmod (lift_f f) in

  let todo =
    match concl.f_node with
    | FhoareF hf when (is_empty_poe (hf_po hf).hsi_inv) ->
      Some (lift_ss_inv t_hoareF_notmod, cond_hoareF_notmod ~mk_other tc (lower_f (hf_po hf)))
    | FhoareS hs when (is_empty_poe (hs_po hs).hsi_inv) ->
      Some (lift_ss_inv t_hoareS_notmod, cond_hoareS_notmod ~mk_other tc (lower_f(hs_po hs)))
