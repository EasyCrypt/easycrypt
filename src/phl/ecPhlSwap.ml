...
module LowInternal = struct
  ...
  =
    let (env, s), (_, (start, fin)) = try 
      normalize_cpos_range env info.interval s 
    with InvalidCPos ->
      tc_error_lazy pf (fun fmt ->
        let ppe = EcPrinting.PPEnv.ofenv env in
        Format.fprintf fmt "invalid range: %a" (EcPrinting.pp_codepos_range ppe) info.interval
      )
    in

    let target = try 
      resolve_offset1_from_cpos1_range env (start, fin) info.offset s 
    with InvalidCPos ->
      tc_error_lazy pf (fun fmt ->
        let ppe = EcPrinting.PPEnv.ofenv env in
        Format.fprintf fmt "invalid offset for swap: %a" (EcPrinting.pp_codeoffset1 ppe) info.offset)
    in

    Format.eprintf "Swapping: (start %d, end %d, target %d)@." start fin target;

    match split_by_nmcpr1s 
      (if target <= start 
      then [target; start; fin]
      else 
        let (_, target) = nm_codepos1_range_of_nm_codepos1 target in
        [start; fin; target]
      ) s
    with 
...
let rec process_swap1 (info : (oside * pswap_kind) located) (tc : tcenv1) =
  ...
  if is_equivS concl && Option.is_none side then
    ...
    let offset = EcTyping.trans_codeoffset1 ~memory:(fst me) env pos.offset in

    let interval = match interval, offset with
    | Some interval, _ -> interval
    | None, `Relative i -> 
        codepos_range_of_codepos
