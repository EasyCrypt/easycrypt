...
let t_ehoare_call fpre fpost tc =
  ...
  let tcenv =
    EcPhlSeq.t_ehoare_seq (EcMatching.Position.cpos1 (List.length s.s_node)) wppre tc in
...
let t_ehoare_call_concave f fpre fpost tc =
  ...
  let tcenv =
    EcPhlSeq.t_ehoare_seq (EcMatching.Position.cpos1 (List.length s.s_node))
