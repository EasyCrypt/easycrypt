...
module Core = struct
 ...
 (* -------------------------------------------------------------------- *)
  ...
  let t_equiv_coupling_r side g tc =
    ...
    let tyL = proj_distr_ty env (e_ty muL) in
    let tyR = proj_distr_ty env (e_ty muR) in
    let muL = EcFol.form_of_expr muL in
    let muR = EcFol.form_of_expr muR in
    ...
    let goal =
      ...
      | None ->
        ...
        let iscoupling_pred = map_ts_inv1 (fun g_app ->
                                               f_app
      ...
      | Some side ->
        ...
        let dmap_pred = map_ts_inv1
                          (fun g -> f_eq (f_app (f_op dmap_op [tyL; tyR] dmap_ty) [muL; g] (tdistr tyR)) muR) g in
