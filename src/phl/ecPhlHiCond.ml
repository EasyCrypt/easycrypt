...
let process_cond (info : EcParsetree.pcond_info) tc =
  let default_if (i : codegap1 option) s =
    ofdfl (fun _ -> GapBefore (cpos1 (tc1_pos_last_if tc s))) i in
  ...
  | `Seq (side, (i1, i2), f) ->
    ...
    let i1 = Option.map (fun i1 -> EcLowPhlGoal.tc1_process_codegap1 tc (side, i1)) i1 in
    let i2 = Option.map (fun i2 -> EcLowPhlGoal.tc1_process_codegap1 tc (side, i2)) i2 in
  ...
  | `SeqOne (s, i, f1, f2) ->
    let es = tc1_as_equivS tc in
    let i = Option.map (fun i1 -> EcLowPhlGoal.tc1_process_codegap1 tc (Some s, i1)) i in
