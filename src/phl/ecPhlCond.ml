...
module LowInternal = struct
 let t_gen_cond side e tc =
   ...
   let t_sub b tc =
     FApi.t_on1seq 0
       (EcPhlRCond.t_rcond side b (EcMatching.Position.cpos 1))
       (FApi.t_seqs
        ...
        FApi.t_on1seq 1 (t_cut fiff)
          (t_intros_i_seq [hiff]
             (FApi.t_seqsub
                ...
                [FApi.t_seqsub
                   (EcPhlRCond.Low.t_equiv_rcond `Right true (EcMatching.Position.cpos 1))
                   [t_aux; t_clear hiff];
                 FApi.t_seqsub
                   (EcPhlRCond.Low.t_equiv_rcond `Right false (EcMatching.Position.cpos 1))
                   [t_aux; t_clear hiff]]))
          tc
...
module LowMatchInternal : sig
  ...
  let t_gen_match (side : side option) (tc : tcenv1) : tcenv =
    ...
    let onsub (i : int) (tc : tcenv1) =
      ...
      tc
        |> EcPhlRCond.t_rcond_match side cname (EcMatching.Position.cpos 1)
        @+ [discharge; clean]
    in
