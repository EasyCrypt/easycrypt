...
module LowInternal = struct
 ...
 let t_gen_cond ?(t_finalize=t_finalize) side e tc =
   ...
   let t_sub b tc =
     FApi.t_on1seq 0
       (EcPhlRCond.t_rcond side b (EcMatching.Position.cpos1 0))
       (FApi.t_seqs
        ...
        FApi.t_on1seq 1 (t_cut fiff)
          (t_intros_i_seq [hiff]
             (FApi.t_seqsub
                ...
                [FApi.t_seqsub
                   (EcPhlRCond.Low.t_equiv_rcond `Right true EcMatching.Position.cpos1_first)
                   [t_aux; t_clear hiff];
                 FApi.t_seqsub
                   (EcPhlRCond.Low.t_equiv_rcond `Right false EcMatching.Position.cpos1_first)
                   [t_aux; t_clear hiff]]))
          tc
...
module LowMatchInternal : sig
  ...
  let t_gen_match (side : side option) (tc : tcenv1) : tcenv =
    ...
    let onsub (i : int) (tc : tcenv1) =
      ...
      tc
        |> EcPhlRCond.t_rcond_match side cname EcMatching.Position.cpos1_first
        @+ [discharge; clean]
    in
...
let t_equiv_match_same_constr tc =
  ...
  let get_eqv_cond ((c, _), ((cl, _), (cr, _))) =
    ...
    let copl = f_op cop ~tyargs:tyl (toarrow (List.snd cl) fl.inv.f_ty) in
    let copr = f_op cop ~tyargs:tyr (toarrow (List.snd cr) fr.inv.f_ty) in
