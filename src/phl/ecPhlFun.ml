...
(* -------------------------------------------------------------------- *)
let subst_pre env fs (m : memory) subst =
  let fresh ov =
  ...
  let subst =
    ...
    PVM.add env pv_arg m (f_tuple v) subst in

  let subst =
    if is_none fs.fs_qarg then
      subst
    else
      let v = List.map (fun v -> f_pvloc (fresh v) m) fs.fs_qnames in
      PVM.add env pv_qarg m (f_tuple v) subst in
...
let t_equivF_fun_def_r tc =
  ...
  let s = PVM.empty in
  let s = PVM.add env (pv_res fsigl) ml fresl s in
...
module FunAbsLow = struct
  ...
  let equivF_abs_spec pf env fl fr inv =
    ...
    let ospec o_l o_r =
      ...
      let eq_res =
        f_eqres
          (fs_quantum fo_l.f_sig)
          fo_l.f_sig.fs_ret ml
          fo_r.f_sig.fs_ret mr in
...
module ToCodeLow = struct
  ...
  let to_code env f m =
    ...
    let doargs prefix =
      let doarg i ov =
        match ov.ov_name with
        | None   -> { ov with ov_name = Some (Printf.sprintf "%s%d" prefix (i + 1)) }
        | Some _ -> ov
      in fun ovs -> List.mapi doarg ovs in

    let (me, args) =
      let args = doargs "arg" fd.f_sig.fs_anames in
      EcMemory.bindall_fresh args me in
    ...
    let (me, qargs) =
      ...
      | `Quantum ->
         let qargs = doargs "qarg" fd.f_sig.fs_qnames in
         let me, qargs = EcMemory.bindall_fresh qargs me in
         (me, Some qargs) in
    ...
    let me, res =
      let res = {
        ov_quantum = quantum;
        ov_name    = Some "r";
        ov_type    = fd.f_sig.fs_ret;
      } in
      EcMemory.bind_fresh res me in

    let res = var_of_ovar res in

    let var ov = var_of_ovar ov in

    let args  = List.map var args in
    let qargs = omap (List.map var) qargs in

    let eargs  =
      List.map
        (fun v -> e_var (pv_loc (v_quantum v) (v_name v)) (v_type v))
        args in
    ...
    let icall =
      let lv = LvVar (pv_loc res.v_quantum res.v_name, res.v_type) in
      i_call (Some lv, f, eargs, qeargs) in

    (fd.f_sig, me, stmt [icall], res, args, qargs)
  ...
  let add_var_tuple env vfrom mfrom vs me s =
    let vs =
      List.map
        (fun v -> f_pvar (pv_loc v.v_quantum v.v_name) v.v_type (fst me))
        vs
    in PVM.add env vfrom mfrom (f_tuple vs) s

  let maybe_add_var_tuple env vfrom mfrom vs me s =
    match vs with
    | None -> s
    | Some vs -> add_var_tuple env vfrom mfrom vs me s
end
...
let t_fun_to_code_hoare_r tc =
  ...
  let hf = tc1_as_hoareF tc in
  let f = hf.hf_f in
  let fsig, m, st, r, a, qa = ToCodeLow.to_code env f mhr in
  let spr   = PVM.empty in
  let spr   = ToCodeLow.add_var_tuple env pv_arg mhr a m spr in
  let spr   = ToCodeLow.maybe_add_var_tuple env pv_qarg mhr qa m spr in
...
let t_fun_to_code_equiv_r tc =
  ...
  let sigl, ml, sl, rl, al, qal = ToCodeLow.to_code env fl mleft in
  let sigr, mr, sr, rr, ar, qar = ToCodeLow.to_code env fr mright in
  let spr =
    let s = PVM.empty in
    let s = ToCodeLow.add_var_tuple env pv_arg mleft al ml s in
    let s = ToCodeLow.maybe_add_var_tuple env pv_qarg mleft qal ml s in
    let s = ToCodeLow.add_var_tuple env pv_arg mright ar mr s in
    let s = ToCodeLow.maybe_add_var_tuple env pv_qarg mright qar mr s in
    s in
 let spo =
    let s = PVM.empty in
    let s = ToCodeLow.add_var env (pv_res sigl) mleft rl ml s in
    let s = ToCodeLow.add_var env (pv_res sigr) mright rr mr s in
    s in
