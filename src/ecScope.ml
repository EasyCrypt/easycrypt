...
module Auto = struct
  ...
  let bind_hint scope ~(irreducible) ~local ~level ?base axioms =
    let item = EcTheory.mkitem EcTheory.import0 (Th_auto { level; base; axioms; locality=local; irreducible } ) in
    { scope with sc_env = EcSection.add_item item scope.sc_env }
  ...
  let add_hint scope hint =
    ...
    bind_hint scope ~irreducible:hint.ht_irreducible ~local:hint.ht_local ~level:hint.ht_prio ?base names
end
...
module Op = struct
  ...
  let add (scope : scope) (op : poperator located) =
    ...
    =
      ...
      List.fold_left
        (fun scope base ->
            Auto.bind_hint ~local:(local_of_locality lc) ~irreducible:false ~level:0 ~base scope [axpath])
        scope bases
