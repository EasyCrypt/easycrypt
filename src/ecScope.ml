...
module Op = struct
  ...
  module Sem = struct
    ...
    let rec translate_i (env : senv) (cont : senv -> mode * expr) (i : instr) =
      ...
      let wr =
        let do1 (pv, ty) =
          ...
          | PVloc  (`Classical, x) -> (x, ty)
          | PVloc (`Quantum, _) -> raise SemNotSupported in
      ...
      | Scall (Some lv, ({ x_top = { m_top = `Concrete (p, _) }; x_sub = f } as xp), args, None)  ->
    ...
    and translate_forloop (env : senv) (cont : senv -> mode * expr) (s : stmt) =
      ...
      match s.s_node with
      | { i_node = Sasgn (LvVar (PVloc (`Classical, x), xty), e) } :: { i_node = Swhile (c, body) } :: s_tail ->
         ...
         let inc, body =
           ...
           | Sasgn (LvVar (PVloc (`Classical, y), _), ic) ->
              if x <> y then
                raise SemNotSupported
              else begin
                match ic.e_node with
                | Eapp ({ e_node = Eop (op, []) }, [{ e_node = Evar (PVloc (`Classical, y')) }; { e_node = Eint inc }])
                     when    y = y'
           ...
           if BI.gt inc BI.one then begin
             let mx =
               e_app
                 ...
                 [e_int inc; e_var (pv_loc `Classical x) tint] tint in
             let subst = EcPV.Mpv.add env.env (pv_loc `Classical x) mx EcPV.Mpv.empty in
             EcPV.Mpv.issubst env.env subst body
           end else body in

         let bd =
           match c.e_node with
           | Eapp ({ e_node = Eop (op, []) }, [{ e_node = Evar (PVloc (`Classical, y)) }; bd])
                when    x = y
         ...
         if EcPV.PV.mem_pv env.env (pv_loc `Classical x) wr then
           raise SemNotSupported;
...
module Mod = struct
  ...
  let add_concrete (scope : scope) (mode : Ax.mode) (lc : _) (ptm : pmodule_def) =
    assert (scope.sc_pr_uc = None);

    let m, goals =
      let g = ref [] in
      let m = TT.transmod (env scope) g ~attop:true ptm in
      (m, !g) in
  ...
  let declare (scope : scope) (m : pmodule_decl) =
    ...
    let tysig = trans_restr_for_modty (env scope) (loc m.ptm_name) tysig modty.pmty_mem in
    { scope with sc_env = EcSection.add_decl_mod name tysig scope.sc_env }

  let add (scope : scope) (mode : Ax.mode) (m : pmodule_def_or_decl) =
    match m with
    | { ptm_locality = lc; ptm_def = `Concrete def } ->
      add_concrete scope mode lc def
...
module ModType = struct
  ...
  let add (scope : scope) (mode : Ax.mode) (intf : pinterface) =
    assert (scope.sc_pr_uc = None);
    let tysig, goals =
      let goals = ref [] in
      let tysig = EcTyping.transmodsig (env scope) goals intf in
      tysig, !goals
    in
