...
and process1_case (_ : ttenv) (doeq, opts, gp) (tc : tcenv1) =
  ...
  in
    ...
    | FaequivS _ when not opts.cod_ambient ->
        let fp = TTC.tc1_process_aprhl_formula tc (form_of_gp ()) in
        EcPhlCase.t_aequiv_case fp tc
...
and process1_phl (_ : ttenv) (t : phltactic located) (tc : tcenv1) =
  let (tx : tcenv1 -> tcenv) =
    ...
    | Paprhl (APwEqBad info)    -> EcPhlAequiv.t_pweq_bad info
