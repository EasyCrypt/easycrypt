...
and game = {
  g_name : string;
  g_pos : pos;
  g_interface : igamesig;
  g_subinterface : string option;
  g_body : game_item context;
}
and game_functor = {
  gf_pos : pos;
  gf_iparams : (string * igame) list;
  gf_game : game;
}
and igamesig_item =
  [ `FunctionDecl of ifct ]
and igamesig = {
  gi_context : igamesig_elt context;
}
and igame = {
  gi_name  : string;
  gi_pos   : pos;
...
type theory = {
  th_name : string;
  th_body : theory_stm context;
}

module GameUtils =
  let collect_vars (m : game) =     (* FIXME: subgames *)
    let vars =
      Context.fold
        (fun vars _ o ->
          match o with `Variable v -> v :: vars | _ -> vars)
        [] m.g_context
    in
      List.rev vars
end
