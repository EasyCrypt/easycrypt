...
=
  ...
  | FhoareF hf ->
      ...
      let hf_poe = List.map (fun (e,(f:ss_inv)) -> e,f.inv) (hf_poe hf) in
  ...
  | FhoareS hs ->
      ...
      let hs_poe = List.map (fun (e,(f:ss_inv)) -> e,f.inv) (hs_poe hs) in
...
(* -------------------------------------------------------------------- *)
let pp_added_except (ppe : PPEnv.t) fmt e =
  let ppe = PPEnv.add_locals ppe (List.map fst e.e_typargs) in
  match e.e_typargs with
  | [] -> Format.fprintf fmt ": @[<hov 2>@]"
  | ts  -> Format.fprintf fmt "@[<hov 2>%a@]" (pp_tyvarannot ppe) ts
...
type ppnode1 = [
  ...
  | `Raise    of (EcPath.path * EcTypes.expr list)
...
let at (ppe : PPEnv.t) n i =
  ...
  | Sraise (e,es)    , 0 -> Some (`Raise (e,es)    , `P, [])
...
let pp_i_raise (_ppe : PPEnv.t) fmt ((e,_es):EcPath.path * expr list) =
  Format.fprintf fmt "raise %a" pp_path e
...
(*     Format.fprintf fmt "%a%!" (pp_post ppef ?prpo) (hs_po hs).inv *)
  ...
  Format.fprintf fmt "@\n%!";
  let hs_poe = List.map (fun (e,(f:ss_inv)) -> e,f.inv) (hs_poe hs) in
  Format.fprintf fmt "%a%!" (pp_poe ppef ?prpo) hs_poe
...
let rec pp_instr_r (ppe : PPEnv.t) fmt i =
  ...
  | Sraise _ ->
    Format.fprintf fmt "@[<hov 2>assert TODO@];"
