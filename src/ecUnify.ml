...
let unify_core (env : EcEnv.env) (uf : UF.t) t1 t2 =
  ...
  let ocheck uf i t =
    let map = ref Suid.empty in

    let rec doit t =
      match t.ty_node with
      | Tunivar i' when uid_equal i i'   -> true
      | Tunivar i' when Suid.mem i' !map -> false
      | Tunivar i' -> begin
          ...
          | Some t -> begin
            match doit t with
            | true  -> true
            | false -> map := Suid.add i' !map; false
          end
      end
...
(* -------------------------------------------------------------------- *)
let close (uf : UF.t) =
  let map = ref Muid.empty in

  let rec doit t =
    match t.ty_node with
    | Tunivar i -> begin
       match Muid.find_opt i !map with
       | Some t -> t
       | None   -> begin
         let t =
           match UF.data i uf with
