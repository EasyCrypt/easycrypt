...
and subst_eop ety tys args (tyids, e) =
  let s = add_tyvars empty tyids (List.fst tys) in (* FIXME: TC *)
...
and subst_etyarg (s : subst) ((ty, tcws) : etyarg) : etyarg =
  (subst_ty s ty, List.map (subst_tcw s) tcws)
...
(* -------------------------------------------------------------------- *)
and subst_tcw (s : subst) ((tcw, p) : tcwitness) =
  let tcw =
    List.map
      (fun (ty, tcws) -> (subst_ty s ty, subst_tcws s tcws))
      tcw in
  let p = subst_path s p in
...
let subst_typeclass (s : subst) (tc : typeclass) =
  { tc_name = subst_path s tc.tc_name;
    tc_args = List.map (subst_ty s) tc.tc_args; }
...
let subst_instance (s : subst) tci =
  ...
  | `General (tc, syms) ->
     ...
     let syms =
       Option.map
         (Mstr.map (fun (p, tys) -> (subst_path s p, List.map (subst_ty s) tys)))
         syms in
     `General (tc, syms)
