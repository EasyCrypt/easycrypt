...
(* A trailing `+` on an idxvar name opts in to a [0 <= n =>]
   hypothesis in the proof goal (inserted inside the lemma's
   [pa_vars] forall). Without `+`, no hypothesis is added. Marks
   never leak into the saved [ax_spec] — only the proof goal
   sees them. *)
...
(* [pa_vars] auto-intro still works: the implications are pushed
   inside the forall, not at the top. *)
lemma idx_with_args {n+} (xs : int vec<:n>) : 0 <= n.
proof. move=> Hn. trivial. qed.

(* Without `+`, no hypothesis is injected — the lemma's body must
   be provable as-is. *)
