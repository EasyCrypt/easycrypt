...
(* Index non-negativity is not a binder marker: it is the enforced
   naturals invariant, available as the [Int.ge0_index] axiom. *)
lemma idx_ge0_simple {n} : 0 <= n.
proof. exact ge0_index. qed.

lemma idx_ge0_smt {m n} : 0 <= m + n.
proof. smt(ge0_index). qed.
...
(* ------------------------------------------------------------------ *)
(* section-declared indices: duplicates are rejected
   (message: `duplicate declared index' -- hierror embeds locations,
   so we do not exact-match it) *)
section.
declare index {k}.
fail declare index {k}.
end section.

section.
fail declare index {k k}.
end section.

(* ------------------------------------------------------------------ *)
(* nested `<: ... >` applications: the trailing `>>' lexes as one
   operator token; the spaced form parses.  (The unspaced form's
   dedicated parse-error hint -- "`>>' is a single operator token:
   separate the closing brackets" -- is asserted manually; parse
   errors cannot be caught by [fail].) *)
type {n} 'a pvec.
op nw = witness<:int pvec<:3> >.

(* ------------------------------------------------------------------ *)
(* section-declared indices carry NO automatic [0 <= n] premise; the
   fact is obtained explicitly from Int.ge0_index. *)
section.
declare index {k}.
lemma sec_ge0k : 0 <= k + 1.
proof. by have := ge0_index[:k]; smt(). qed.
end section.
